...
func NewProxier(ipt utiliptables.Interface, exec utilexec.Interface, syncPeriod time.Duration, masqueradeAll bool, masqueradeBit int, clusterCIDR string, nodeIP net.IP) (*Proxier, error) {
...
func (proxier *Proxier) OnEndpointsUpdate(allEndpoints []api.Endpoints) {
	...
	activeEndpoints := make(map[proxy.ServicePortName]bool) // use a map as a set staleConnections := make(map[endpointServicePair]bool)
		...
		for i := range svcEndpoints.Subsets {
			...
			for i := range ss.Ports {
				...
				for i := range ss.Addresses {
					...
					glog.V(4).Infof("addr.IP=%s", addr.IP)
					if addr.IP != "" {
						isLocalEndpoint, _ = isLocalEndpointIP(addr.IP + "/24")
						glog.V(4).Infof("isLocalEndpoint=%v", isLocalEndpoint)
					}
...
func (proxier *Proxier) syncProxyRules() {
	...
	for svcName, svcInfo := range proxier.serviceMap {
		...
		glog.V(4).Infof("numLocalEndpoints=%d", numLocalEndpoints)
...
func isLocalEndpointIP(ip string) (bool, error) {
	ifi, err := net.InterfaceByName("docker0")
	if err != nil {
		return false, err
	}
	glog.V(4).Infof("rpei ifi=%v", ifi)
	_, ipNetDst, err := net.ParseCIDR(ip)
	if err != nil {
		return false, err
	}
	addrs, err := ifi.Addrs()
	...
	for i := range addrs {
		_, ipNetSrc, err := net.ParseCIDR(addrs[i].String())
		if err != nil {
			return false, err
		}
		glog.V(4).Infof("rpei ipNetSrc.IP=%v", ipNetSrc.IP)
		if ipNetSrc.IP.Equal(ipNetDst.IP) {
			return true, nil
		}
	}
	return false, nil
